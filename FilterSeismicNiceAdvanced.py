...
def get_quakes1(csv_times, csv_data, ax, interval, mean_r, incre, min_terval):
    ...
    for n in range(0, len(csv_data), interval):
        ...
        if (local_av > av * mean_r):
            if (seismic_act[-1][1] != "") and (local_av > incre*averages[-1]):
            ...
            if seismic_act[-1][1] == "":
                if csv_times[n]-seismic_act[-1][0] > csv_times[interval*min_terval]:
    ...
    return seismic_act, averages, av

def analyze_moon(file_name, cat_file):    
    cat = pd.read_csv(cat_file)

    start_t = cat.iloc[1]["time_abs(%Y-%m-%dT%H:%M:%S.%f)"]
    ...
    seismic_act, averages, av = get_quakes1(csv_times, csv_data, ax, 4*60*4, 2, 1.2, 6)
...
def analyze_mars(file_name, cat_file):
    ...
    seismic_act, averages, av = get_quakes1(csv_times, csv_data, ax, 20*12, 1.5, 1.2, 8)
...
def download_light_data(analysed, path):  
    ...
    eArr = np.repeat("", len(analysed[0][0])-1)
    eArr = np.concatenate([[analysed[3]], eArr])

    print(analysed[3], eArr, len(eArr), len(analysed[0][0]))
...
def read_and_filter_csv():
    ...
    try:
        ...
        res = analyse(filepath, moon_mars)
        intervalos_tiempo = res[1]
        ...
        return seismic_wave, time, velocity, ratio, res
    except FileNotFoundError:
        print("File not found")


def draw_wave(w, t, v, r, f):
...
if (str(input("Download? (Y/N): ")) == "Y"):
    path = input("Give path to download: ")
    if path == "":
        path = "C:/Users/Usuario/Downloads/NasaSpaceApps2024/DataAnalysed/MoonWave9"
    download_light_data(final[4], path)
